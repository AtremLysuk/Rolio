...
const About: FC = () => {
	...
	useGSAP(() => {
		...
		document.fonts.ready.then(() => {
			if (!isActive) return;
			const textArray = gsap.utils.toArray<HTMLElement>(containerRef.current?.querySelectorAll('[data-text]') || []);
					...
					{aboutItems.map((item, index) => (
						...
						>
							<div className={styles.image}>
								<img
									src={item.imgSrc}
									alt={item.subtitle === 'Кто мы'
										? 'Таня и Рома — создатели пряного оливкового масла R.olio'
										: 'Ингредиенты и процесс приготовления пряного масла'}
									width={433}
									height={314}
									loading="lazy"
								/>
