...
export const MessageModal: FC = () => {
  ...
  useEffect(() => {
    ...
    const focusableItems: HTMLElement[] = Array.from(
      containerRef.current?.querySelectorAll('[data-tab]')
    );
    ...
    const tabListener = (e: KeyboardEvent) => {
      ...
      const index = focusableItems.indexOf(active!);
      const first = focusableItems[0];
      const last = focusableItems[focusableItems.length - 1];

      if (active === last || index === focusableItems.length - 1) {
        e.preventDefault();
        first.focus();
      }
    };

    containerRef.current.addEventListener('keydown', tabListener);

    return () => {
      containerRef.current?.removeEventListener('keydown', tabListener);
    };
  }, []);
    ...
    >
      <div className={styles.inner} ref={containerRef}>
        ...
        <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
          ...
          <button className={styles.sendBtn} type="submit" data-tab>
            ОТПРАВИТЬ
          </button>
        </form>
      </div>
